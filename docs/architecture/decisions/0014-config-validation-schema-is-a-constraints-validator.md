...
- `docs/schemas/api/config.schema.json` — code-first, drift-gated, the _shape_
  contract the UI's generated `Config` type mirrors.
- `internal/config/schema.json` — hand-curated, embedded, the _value-constraint_
...
- The remaining genuine concern is _coverage drift_ — a new `config.Config`
  field could be added without a matching constraint entry, leaving it
  unvalidated. The generated schema already tracks the struct's _shape_ (its
