// Package reporting_test provides comprehensive unit tests for internal functions.
...
func TestGenerateHTML_Internal(t *testing.T) {
	...
	data := &reporting.AggregatedData{
		StartDate:   time.Now().AddDate(0, 0, -7),
		EndDate:     time.Now(),
		DeviceCount: 42,
		VulnCount: reporting.VulnCounts{
			Critical: 5,
			High:     10,
			Medium:   20,
			Low:      15,
...
func TestGenerateJSON_Internal(t *testing.T) {
	...
	report := &reporting.Report{
		...
		Type: reporting.ReportTypePerformance,
	}
...
func TestGeneratePDF_Internal(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "executive report", reportType: reporting.ReportTypeExecutive},
		{name: "vulnerability report", reportType: reporting.ReportTypeVulnerability},
		{name: "detailed report", reportType: reporting.ReportTypeDetailed},
		{name: "performance report", reportType: reporting.ReportTypePerformance},
		{name: "inventory report", reportType: reporting.ReportTypeInventory},
	}
...
func TestAggregateTopIssues_Internal(t *testing.T) {
	...
	data := &reporting.AggregatedData{
		TopIssues: make([]reporting.IssueSummary, 0), // Initialize to empty slice
	}
...
func TestLoadSchedules_Internal(t *testing.T) {
	...
	ss2 := reporting.NewSchedulerService(cfg, store.NewScheduleRepo(db), gs)
...
func TestSaveSchedule_Internal(t *testing.T) {
	...
	schedule := &reporting.ScheduledReport{
		...
		Parameters: reporting.ReportParams{
			Filters: map[string]string{"device_type": "server"},
		},
		Recipients: []reporting.Recipient{
	...
	ss2 := reporting.NewSchedulerService(cfg, store.NewScheduleRepo(db), gs)
	schedule2 := &reporting.ScheduledReport{
	...
	tests := []struct {
	...
	}{
		{
			...
			report: &reporting.Report{
				...
				Type:      reporting.ReportTypeExecutive,
				Format:    reporting.FormatPDF,
				Status:    reporting.StatusPending,
				CreatedAt: now,
			},
		},
		{
			...
			report: &reporting.Report{
				...
				Type:        reporting.ReportTypeVulnerability,
				Format:      reporting.FormatHTML,
		...
		},
		{
			...
			report: &reporting.Report{
				...
				Type:   reporting.ReportTypeDetailed,
				Format: reporting.FormatCSV,
				Status: reporting.StatusPending,
				Parameters: reporting.ReportParams{
...
func TestReportsPath_Internal(t *testing.T) {
	...
	assert.Equal(t, reporting.DefaultReportsPath, defaultPath)
...
func TestGetTrends_AllMetrics(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "latency daily", metric: "latency", period: reporting.PeriodDaily},
		{name: "latency weekly", metric: "latency", period: reporting.PeriodWeekly},
		{name: "latency monthly", metric: "latency", period: reporting.PeriodMonthly},
		{name: "latency default", metric: "latency", period: "invalid"},
		{name: "bandwidth daily", metric: "bandwidth", period: reporting.PeriodDaily},
		{name: "bandwidth weekly", metric: "bandwidth", period: reporting.PeriodWeekly},
		{name: "devices daily", metric: "devices", period: reporting.PeriodDaily},
		{name: "devices weekly", metric: "devices", period: reporting.PeriodWeekly},
		{name: "devices monthly", metric: "devices", period: reporting.PeriodMonthly},
	}
...
func TestGetTrends_UnsupportedMetric(t *testing.T) {
	...
	ctx := context.Background()
	_, err := as.GetTrends(ctx, "unsupported_metric", reporting.PeriodWeekly)
	require.Error(t, err)
...
func TestGeneratorService_Export_Formats(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "JSON export", format: reporting.FormatJSON},
		{name: "CSV export", format: reporting.FormatCSV},
	}
...
func TestIncidentSummary_Struct(t *testing.T) {
	...
	incident := reporting.IncidentSummary{
		ID:         "inc-1",
		Title:      "Test Incident",
		Severity:   "critical",
		Status:     "resolved",
...
func TestPerformanceMetrics_Struct(t *testing.T) {
	metrics := reporting.PerformanceMetrics{
		AvgLatencyMs:     15.5,
		AvgPacketLoss:    0.5,
		AvgBandwidthMbps: 100.0,
		UptimePercent:    99.9,
