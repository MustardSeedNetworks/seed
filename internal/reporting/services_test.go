	...
	tests := []struct {
	...
	}{
		{
			...
			template: &reporting.Template{
				...
				Type:        reporting.ReportTypeCustom,
				Formats:     []reporting.ExportFormat{reporting.FormatPDF, reporting.FormatHTML},
			},
			wantErr: false,
		},
		{
			...
			template: &reporting.Template{
				...
				Type:    reporting.ReportTypeCustom,
				Formats: []reporting.ExportFormat{reporting.FormatJSON},
			},
			wantErr: false,
		},
		{
		...
		},
		{
			...
			template: &reporting.Template{
				...
				Formats: []reporting.ExportFormat{reporting.FormatPDF},
			},
			wantErr:   true,
...
func TestAggregatorService_Aggregate(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "daily period", period: reporting.PeriodDaily},
		{name: "weekly period", period: reporting.PeriodWeekly},
		{name: "monthly period", period: reporting.PeriodMonthly},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			case reporting.PeriodWeekly, "invalid":
...
func TestAggregatorService_AggregateWithData(t *testing.T) {
	...
	data, err := as.Aggregate(ctx, reporting.PeriodWeekly, "", "")
	require.NoError(t, err)
	require.NotNil(t, data)
...
func TestAggregatorService_GetTrends(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "latency daily", metric: "latency", period: reporting.PeriodDaily, wantErr: false},
		{name: "latency weekly", metric: "latency", period: reporting.PeriodWeekly, wantErr: false},
		{name: "latency monthly", metric: "latency", period: reporting.PeriodMonthly, wantErr: false},
		{name: "bandwidth daily", metric: "bandwidth", period: reporting.PeriodDaily, wantErr: false},
		{name: "devices weekly", metric: "devices", period: reporting.PeriodWeekly, wantErr: false},
		{name: "unsupported metric", metric: "unsupported", period: reporting.PeriodWeekly, wantErr: true},
	}
...
func TestSchedulerService_Update(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "update name",
			modifyFn: func(s *reporting.ScheduledReport) {
				s.Name = "Updated Name"
			},
			wantErr: false,
		},
		{
			...
			modifyFn: func(s *reporting.ScheduledReport) {
				s.Schedule.Frequency = reporting.FrequencyWeekly
...
func TestCalculateNextRun(t *testing.T) {
	...
	tests := []struct {
		name       string
		frequency  reporting.ScheduleFrequency
		dayOfWeek  *int
		dayOfMonth *int
		hour       int
	}{
...
func TestGeneratorService_Generate(t *testing.T) {
	...
	tests := []struct {
		...
		params     *reporting.ReportParams
	}{
		{
			name:       "executive PDF",
			reportType: reporting.ReportTypeExecutive,
			format:     reporting.FormatPDF,
			params:     nil,
		},
		{
			name:       "vulnerability HTML",
			reportType: reporting.ReportTypeVulnerability,
			format:     reporting.FormatHTML,
			params:     nil,
		},
		{
			name:       "performance CSV",
			reportType: reporting.ReportTypePerformance,
			format:     reporting.FormatCSV,
			params:     nil,
		},
		{
			name:       "inventory JSON",
			reportType: reporting.ReportTypeInventory,
		...
		{
			name:       "detailed PDF",
			reportType: reporting.ReportTypeDetailed,
			format:     reporting.FormatPDF,
			params:     nil,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.True(t, report.Status == reporting.StatusPending ||
				report.Status == reporting.StatusGenerating ||
				report.Status == reporting.StatusComplete,
...
func TestGeneratorService_GenerateFromTemplate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			templateID: "executive",
			format:     reporting.FormatCSV, // Executive doesn't support CSV
			wantErr:    true,
...
func TestGeneratorService_Export(t *testing.T) {
	...
	tests := []struct {
		name      string
		request   *reporting.ExportRequest
		wantErr   bool
		errSubstr string
	}{
		{
...
func TestGeneratorService_ListReports(t *testing.T) {
	...
	for range 3 {
		_, err := gs.Generate(ctx, reporting.ReportTypeExecutive, reporting.FormatPDF, nil)
		require.NoError(t, err)
	}
...
func TestGeneratorService_GetReport(t *testing.T) {
	...
	// Generate a report
	report, err := gs.Generate(ctx, reporting.ReportTypeExecutive, reporting.FormatPDF, nil)
	require.NoError(t, err)
...
func TestGeneratorService_UnsupportedFormats(t *testing.T) {
	...
	unsupportedFormats := []reporting.ExportFormat{
		reporting.FormatExcel,
		reporting.FormatMarkdown,
	}

	for _, format := range unsupportedFormats {
		t.Run(string(format), func(t *testing.T) {
			report, err := gs.Generate(ctx, reporting.ReportTypeExecutive, format, nil)
			...
			if retrieved != nil {
				...
				assert.Equal(t, reporting.StatusFailed, retrieved.Status)
			}
		})
	}
}
...
func TestReportParams_DateRangeCalculation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			report, err := gs.Generate(ctx, reporting.ReportTypeExecutive, reporting.FormatJSON, params)
			require.NoError(t, err)
			require.NotNil(t, report)
		})
	}
...
func TestExportResult_Fields(t *testing.T) {
	...
	result, err := gs.Export(ctx, &reporting.ExportRequest{
		Type:   "devices",
		Format: reporting.FormatJSON,
	})
	require.NoError(t, err)
	...
	assert.Equal(t, reporting.FormatJSON, result.Format)
	assert.False(t, result.CreatedAt.IsZero())
	assert.True(t, result.ExpiresAt.After(result.CreatedAt))
...
func TestTypes(t *testing.T) {
	// Report types
	reportTypes := []reporting.ReportType{
		reporting.ReportTypeExecutive,
		reporting.ReportTypeDetailed,
		reporting.ReportTypeVulnerability,
		reporting.ReportTypeCompliance,
		reporting.ReportTypeInventory,
		reporting.ReportTypePerformance,
		reporting.ReportTypeIncident,
		reporting.ReportTypeCustom,
	}
	...
	// Export formats
	exportFormats := []reporting.ExportFormat{
		reporting.FormatPDF,
		reporting.FormatHTML,
		reporting.FormatCSV,
	...
	// Report statuses
	statuses := []reporting.ReportStatus{
		reporting.StatusPending,
		reporting.StatusGenerating,
		reporting.StatusComplete,
		reporting.StatusFailed,
		reporting.StatusExpired,
	}
	...
	frequencies := []reporting.ScheduleFrequency{
...
func TestErrors(t *testing.T) {
	...
	require.Error(t, reporting.ErrNotImplemented)
	assert.Contains(t, reporting.ErrNotImplemented.Error(), "not implemented")
}
...
func TestDefaultConstants(t *testing.T) {
	assert.Equal(t, "data/reports", reporting.DefaultReportsPath)
	assert.Equal(t, 7*24*time.Hour, reporting.DefaultReportTTL)
	assert.Equal(t, "daily", reporting.PeriodDaily)
	assert.Equal(t, "weekly", reporting.PeriodWeekly)
	assert.Equal(t, "monthly", reporting.PeriodMonthly)
}
