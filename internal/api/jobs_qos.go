...
// jobs_qos.go registers #400's DSCP preservation check as two job kinds, one
// per host: qos-listen holds a port open on the far side for a bounded window
// and reads the marking every probe arrives with; qos-send puts the marked
// probes on the wire. They are jobs rather than routes for the same reason as
// multicast-listen: each holds a socket for seconds, and the runner already
// provides the cancel, the concurrency ceiling and the handle a caller polls.
...
// need at least a port.
var errQoSParams = errors.New("qos job requires params")

// qosSend and qosListen are qos.Send and qos.Listen, behind seams so the
// kinds are testable without a socket.
type (
...
// newQoSHandler returns the job Handler for one half. A cancelled listen or
// send returns what it has, so the job succeeds with a partial result.
...
// registerQoSKinds registers both halves with injectable implementations.
func (s *Server) registerQoSKinds(send qosSend, listen qosListen) {
