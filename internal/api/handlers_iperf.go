...
// iPerf timing constants.
const (
	// iperfTimeoutPaddingSec is the additional time beyond test duration for iPerf operations.
	iperfTimeoutPaddingSec = 30

	// iperfSuggestionDialTimeoutMs is the timeout in milliseconds for probing iPerf servers.
	iperfSuggestionDialTimeoutMs = 400

	// iperfMillisecondsPerSecond is the conversion factor from seconds to milliseconds.
	iperfMillisecondsPerSecond = 1000

	// iperfMaxSuggestions is the maximum number of iperf server suggestions to return.
	iperfMaxSuggestions = 10
)
...
func (s *Server) handleIperfSuggestions(w http.ResponseWriter, r *http.Request) {
	...
	for _, d := range devices {
		...
		start := time.Now()
		dialer := net.Dialer{Timeout: iperfSuggestionDialTimeoutMs * time.Millisecond}
		conn, err := dialer.DialContext(r.Context(), "tcp", addr)
		if err != nil {
			continue
		}
		latency := time.Since(start).Seconds() * iperfMillisecondsPerSecond
		_ = conn.Close()
		...
		if len(suggestions) >= iperfMaxSuggestions {
			break
		}
	}
