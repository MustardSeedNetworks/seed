// Package services_test provides comprehensive tests for the services module.
...
func runConcurrentServiceAccess(t *testing.T, module *services.Module, goroutines, iterations int) {
	t.Helper()
...
func TestNewLinkService(t *testing.T) {
	t.Parallel()
	cfg := config.DefaultConfig()
	service := services.NewLinkService(cfg)
...
func TestNewCableService(t *testing.T) {
	t.Parallel()
	cfg := config.DefaultConfig()
	service := services.NewCableService(cfg)
...
func TestNewDHCPService(t *testing.T) {
	t.Parallel()
	cfg := config.DefaultConfig()
	service := services.NewDHCPService(cfg)
...
func TestNewDNSService(t *testing.T) {
	t.Parallel()
	cfg := config.DefaultConfig()
	service := services.NewDNSService(cfg)
...
func TestNewGatewayService(t *testing.T) {
	t.Parallel()
	cfg := config.DefaultConfig()
	service := services.NewGatewayService(cfg)
...
func TestNewSNMPService(t *testing.T) {
	t.Parallel()
	cfg := config.DefaultConfig()
	service := services.NewSNMPService(cfg)
...
func TestNewPerformanceService(t *testing.T) {
	t.Parallel()
	cfg := config.DefaultConfig()
	service := services.NewPerformanceService(cfg)
...
func TestNewVLANService(t *testing.T) {
	t.Parallel()
	cfg := config.DefaultConfig()
	service := services.NewVLANService(cfg)
...
func TestServiceConstructorsTableDriven(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:        "NewLinkService",
			constructor: func(c *config.Config) any { return services.NewLinkService(c) },
			wantNil:     false,
		},
		{
			name:        "NewCableService",
			constructor: func(c *config.Config) any { return services.NewCableService(c) },
			wantNil:     false,
		},
		{
			name:        "NewDHCPService",
			constructor: func(c *config.Config) any { return services.NewDHCPService(c) },
			wantNil:     false,
		},
		{
			name:        "NewDNSService",
			constructor: func(c *config.Config) any { return services.NewDNSService(c) },
			wantNil:     false,
		},
		{
			name:        "NewGatewayService",
			constructor: func(c *config.Config) any { return services.NewGatewayService(c) },
			wantNil:     false,
		},
		{
			name:        "NewSNMPService",
			constructor: func(c *config.Config) any { return services.NewSNMPService(c) },
			wantNil:     false,
		},
		{
			name:        "NewPerformanceService",
			constructor: func(c *config.Config) any { return services.NewPerformanceService(c) },
			wantNil:     false,
		},
		{
			name:        "NewVLANService",
			constructor: func(c *config.Config) any { return services.NewVLANService(c) },
			wantNil:     false,
		},
	}
...
func TestLinkServiceGetStatusNotInitialized(t *testing.T) {
	...
	if !errors.Is(err, services.ErrNotInitialized) {
...
func TestErrorsAreDistinct(t *testing.T) {
	t.Parallel()
	errs := []error{
		services.ErrNotImplemented,
		services.ErrNotInitialized,
		services.ErrNotSupported,
		services.ErrTestFailed,
	}
...
func TestConstantsValues(t *testing.T) {
	...
	tests := []struct {
		...
		{"DefaultInterface", services.DefaultInterfaceConst, "eth0"},
		{"InterfaceStateWaitMs", services.InterfaceStateWaitMsConst, 100},
		{"SNMPTimeticksPerSecond", services.SNMPTimeticksPerSecondConst, 100},
		{"DefaultIPerfPort", services.DefaultIPerfPortConst, 5201},
		{"DefaultIPerfDurationSec", services.DefaultIPerfDurationSecConst, 10},
	}
...
func TestJoinAddressesTableDriven(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result := services.JoinAddresses(tt.addrs)
...
func TestConvertGatewayStatusTableDriven(t *testing.T) {
	...
	tests := []struct {
		...
		{"success", "success", services.HealthStatusHealthy},
		{"warning", "warning", services.HealthStatusDegraded},
		{"error", "error", services.HealthStatusUnhealthy},
		{"unknown", "unknown", services.HealthStatusUnknown},
		{"empty string", "", services.HealthStatusUnknown},
		{"invalid status", "invalid", services.HealthStatusUnknown},
		{"uppercase SUCCESS", "SUCCESS", services.HealthStatusUnknown}, // case sensitive
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result := services.ConvertGatewayStatus(tt.status)
			if result != tt.expected {
...
func TestConvertGatewayStatusExportedTableDriven(t *testing.T) {
	...
	tests := []struct {
		...
		{"error", "error", services.HealthStatusUnhealthy},
		{"unknown", "unknown", services.HealthStatusUnknown},
		{"empty", "", services.HealthStatusUnknown},
		{"invalid", "foo", services.HealthStatusUnknown},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result := services.ConvertGatewayStatusExported(tt.status)
			if result != tt.expected {
...
func TestConvertCableStatusExportedTableDriven(t *testing.T) {
	...
	tests := []struct {
		...
		{"ok", "ok", services.CableStatusOK},
		{"open", "open", services.CableStatusOpen},
		{"short", "short", services.CableStatusShort},
		{"impedance", "impedance", services.CableStatusImpedance},
		{"unknown", "unknown", services.CableStatusUnknown},
		{"empty", "", services.CableStatusUnknown},
		{"invalid", "foo", services.CableStatusUnknown},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result := services.ConvertCableStatusExported(tt.status)
			if result != tt.expected {
...
func TestMakeLinkStatusHelper(t *testing.T) {
	t.Parallel()
	status := services.MakeLinkStatus("eth0", services.LinkStateUp, "1000Mbps", "full", 1500, "00:11:22:33:44:55")

	if status.Interface != "eth0" {
		t.Errorf("Interface = %q, want %q", status.Interface, "eth0")
	}
	if status.State != services.LinkStateUp {
		t.Errorf("State = %q, want %q", status.State, services.LinkStateUp)
	}
...
func TestMakeLinkStatusDownState(t *testing.T) {
	t.Parallel()
	status := services.MakeLinkStatus("eth1", services.LinkStateDown, "", "", 1500, "00:11:22:33:44:66")

	if status.State != services.LinkStateDown {
		t.Errorf("State = %q, want %q", status.State, services.LinkStateDown)
	}
	if status.Carrier {
...
func TestMakeCableTestResultHelper(t *testing.T) {
	t.Parallel()
	result := services.MakeCableTestResult("eth0", services.CableStatusOK, 25.5)

	if result.Interface != "eth0" {
		t.Errorf("Interface = %q, want %q", result.Interface, "eth0")
	}
	if result.Status != services.CableStatusOK {
		t.Errorf("Status = %q, want %q", result.Status, services.CableStatusOK)
	}
...
func TestMakeDHCPTestResultHelper(t *testing.T) {
	t.Parallel()
	result := services.MakeDHCPTestResult(true, "192.168.1.1", "192.168.1.100", "192.168.1.1")
...
func TestMakeDNSTestResultHelper(t *testing.T) {
	t.Parallel()
	result := services.MakeDNSTestResult("google.com", "8.8.8.8", true, 15.5)
...
func TestMakeGatewayHealthHelper(t *testing.T) {
	t.Parallel()
	health := services.MakeGatewayHealth("192.168.1.1", true, 5.0, 0.0, services.HealthStatusHealthy)
	...
	if health.Status != services.HealthStatusHealthy {
		t.Errorf("Status = %q, want %q", health.Status, services.HealthStatusHealthy)
	}
...
func TestMakeSpeedtestResultHelper(t *testing.T) {
	t.Parallel()
	result := services.MakeSpeedtestResult(100.5, 50.2, 15.0, "TestServer")
...
func TestMakeIPerfResultHelper(t *testing.T) {
	t.Parallel()
	result := services.MakeIPerfResult("tcp", "download", 950.0, 1187.5, 10.0, "192.168.1.100")
...
func TestMakeVLANConfigHelper(t *testing.T) {
	t.Parallel()
	config := services.MakeVLANConfig(100, "Management", "eth0", true)
...
func TestMakeSNMPDeviceHelper(t *testing.T) {
	t.Parallel()
	device := services.MakeSNMPDevice("192.168.1.2", "switch-01", "Cisco IOS")
...
func TestMakeBandwidthSampleHelper(t *testing.T) {
	t.Parallel()
	sample := services.MakeBandwidthSample("eth0", 100.0, 50.0, 15.0)
...
func TestMakeSystemHealthHelper(t *testing.T) {
	t.Parallel()
	health := services.MakeSystemHealth(25.5, 60.0, 45.0)
...
func TestMakeTelemetrySnapshotHelper(t *testing.T) {
	t.Parallel()
	snapshot := services.MakeTelemetrySnapshot()
...
func TestCableServiceTest(t *testing.T) {
	...
	if err != nil && !errors.Is(err, services.ErrNotSupported) && !errors.Is(err, services.ErrTestFailed) {
...
func TestCableServiceTestWithRealInterface(t *testing.T) {
	...
	if err != nil {
		...
		case errors.Is(err, services.ErrTestFailed):
...
func TestConvertGatewayStatusAllCases(t *testing.T) {
	...
	tests := []struct {
		...
		{"success", services.HealthStatusHealthy},
		{"warning", services.HealthStatusDegraded},
		{"error", services.HealthStatusUnhealthy},
		{"unknown", services.HealthStatusUnknown},
		{"", services.HealthStatusUnknown},
		{"something_else", services.HealthStatusUnknown},
		{"SUCCESS", services.HealthStatusUnknown}, // case sensitive
		{"Warning", services.HealthStatusUnknown}, // case sensitive
	}

	for _, tt := range tests {
		t.Run(tt.input, func(t *testing.T) {
			t.Parallel()
			result := services.ConvertGatewayStatus(tt.input)
			if result != tt.expected {
...
func TestLinkStateEnumValues(t *testing.T) {
	t.Parallel()
	states := []services.LinkState{
		services.LinkStateUp,
		services.LinkStateDown,
		services.LinkStateDormant,
		services.LinkStateUnknown,
	}
...
func TestCableStatusEnumValues(t *testing.T) {
	t.Parallel()
	statuses := []services.CableStatus{
		services.CableStatusOK,
		services.CableStatusOpen,
		services.CableStatusShort,
		services.CableStatusImpedance,
		services.CableStatusUnknown,
	}
...
func TestHealthStatusEnumValues(t *testing.T) {
	t.Parallel()
	statuses := []services.HealthStatus{
		services.HealthStatusHealthy,
		services.HealthStatusDegraded,
		services.HealthStatusUnhealthy,
		services.HealthStatusUnknown,
	}
...
func TestConvertCableStatusWithTypeTableDriven(t *testing.T) {
	...
	tests := []struct {
		name     string
		input    services.CableStatusInput
		expected services.CableStatus
	}{
		{"OK", services.CableStatusInputOK, services.CableStatusOK},
		{"Open", services.CableStatusInputOpen, services.CableStatusOpen},
		{"Short", services.CableStatusInputShort, services.CableStatusShort},
		{"ImpedanceMismatch", services.CableStatusInputImpedanceMismatch, services.CableStatusImpedance},
		{"Crosstalk", services.CableStatusInputCrosstalk, services.CableStatusUnknown},
		{"SplitPair", services.CableStatusInputSplitPair, services.CableStatusUnknown},
		{"Unknown", services.CableStatusInputUnknown, services.CableStatusUnknown},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			result := services.ConvertCableStatusWithType(tt.input)
			if result != tt.expected {
...
func TestConvertGatewayStatusWithTypeTableDriven(t *testing.T) {
	...
	tests := []struct {
		name     string
		input    services.GatewayStatusInput
		expected services.HealthStatus
	}{
		{"Success", services.GatewayStatusInputSuccess, services.HealthStatusHealthy},
		{"Warning", services.GatewayStatusInputWarning, services.HealthStatusDegraded},
		{"Error", services.GatewayStatusInputError, services.HealthStatusUnhealthy},
		{"Unknown", services.GatewayStatusInputUnknown, services.HealthStatusUnknown},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			result := services.ConvertGatewayStatusWithType(tt.input)
			if result != tt.expected {
...
func TestConvertPairResultsExportedEmpty(t *testing.T) {
	t.Parallel()
	result := services.ConvertPairResultsExported(nil)
	if result != nil {
		t.Errorf("expected nil for nil input, got %v", result)
	}

	result = services.ConvertPairResultsExported([]services.PairResultInput{})
	if result != nil {
...
func TestConvertPairResultsExportedWithData(t *testing.T) {
	t.Parallel()
	length := 25.5
	input := []services.PairResultInput{
		{Status: services.CableStatusInputOK, LengthM: &length},
		{Status: services.CableStatusInputOpen, LengthM: nil},
		{Status: services.CableStatusInputShort, LengthM: &length},
		{Status: services.CableStatusInputImpedanceMismatch, LengthM: &length},
	}

	result := services.ConvertPairResultsExported(input)
	...
	if result[0].Status != services.CableStatusOK {
		t.Errorf("result[0].Status = %q, want %q", result[0].Status, services.CableStatusOK)
	}
	...
	if result[1].Status != services.CableStatusOpen {
		t.Errorf("result[1].Status = %q, want %q", result[1].Status, services.CableStatusOpen)
	}
	...
	if result[2].Status != services.CableStatusShort {
		t.Errorf("result[2].Status = %q, want %q", result[2].Status, services.CableStatusShort)
	}

	// Check pair 4
	if result[3].Status != services.CableStatusImpedance {
		t.Errorf("result[3].Status = %q, want %q", result[3].Status, services.CableStatusImpedance)
	}
}
...
func TestMakePairResultHelper(t *testing.T) {
	t.Parallel()
	result := services.MakePairResult(1, services.CableStatusOK, 25.5, 100.0)
...
func TestMakeCableTestResultWithPairsHelper(t *testing.T) {
	t.Parallel()
	pairs := []services.PairResult{
		services.MakePairResult(1, services.CableStatusOK, 25.0, 100.0),
		services.MakePairResult(2, services.CableStatusOK, 25.0, 100.0),
		services.MakePairResult(3, services.CableStatusOK, 25.0, 100.0),
		services.MakePairResult(4, services.CableStatusOpen, 10.0, 0.0),
	}

	result := services.MakeCableTestResultWithPairs("eth0", services.CableStatusOpen, 10.0, pairs)

	if result.Interface != "eth0" {
		t.Errorf("Interface = %q, want %q", result.Interface, "eth0")
	}
	if result.Status != services.CableStatusOpen {
		t.Errorf("Status = %q, want %q", result.Status, services.CableStatusOpen)
	}
...
func TestMakeDNSAnswerHelper(t *testing.T) {
	t.Parallel()
	answer := services.MakeDNSAnswer("example.com", "A", "93.184.216.34", 300)
...
func TestMakeDNSTestResultWithAnswersHelper(t *testing.T) {
	t.Parallel()
	answers := []services.DNSAnswer{
		services.MakeDNSAnswer("example.com", "A", "93.184.216.34", 300),
	}

	result := services.MakeDNSTestResultWithAnswers("example.com", "8.8.8.8", true, 15.0, answers)
...
func TestMakeDHCPTestResultFullHelper(t *testing.T) {
	t.Parallel()
	result := services.MakeDHCPTestResultFull(
		true,
		"192.168.1.1",
		"192.168.1.100",
		"192.168.1.1",
...
func TestMakeSNMPInterfaceHelper(t *testing.T) {
	t.Parallel()
	iface := services.MakeSNMPInterface(1, "Gi0/1", "Uplink", "ethernetCsmacd", 1000000000, "up", "up")
...
func TestMakeSNMPVLANHelper(t *testing.T) {
	t.Parallel()
	vlan := services.MakeSNMPVLAN(100, "Management", "active", []int{1, 2, 3, 4})
...
func TestMakeMACTableEntryHelper(t *testing.T) {
	t.Parallel()
	entry := services.MakeMACTableEntry("00:11:22:33:44:55", 1, 100, "dynamic")
...
func TestMakeSNMPDeviceFullHelper(t *testing.T) {
	...
	interfaces := []services.SNMPInterface{
		services.MakeSNMPInterface(1, "Gi0/1", "Uplink", "ethernetCsmacd", 1000000000, "up", "up"),
	}
	vlans := []services.SNMPVLAN{
		services.MakeSNMPVLAN(100, "Management", "active", []int{1}),
	}
	macTable := []services.MACTableEntry{
		services.MakeMACTableEntry("00:11:22:33:44:55", 1, 100, "dynamic"),
	}

	device := services.MakeSNMPDeviceFull(
		"192.168.1.2",
		"core-switch",
		"Cisco IOS",
		"Server Room",
...
func TestMakeGatewayHealthFullHelper(t *testing.T) {
	t.Parallel()
	health := services.MakeGatewayHealthFull(
		"192.168.1.1",
		true,
		5.0,
		0.0,
...
func TestMakeSpeedtestResultFullHelper(t *testing.T) {
	t.Parallel()
	result := services.MakeSpeedtestResultFull(
		500.0,
		250.0,
		15.0,
		2.0,
...
func TestMakeIPerfResultFullHelper(t *testing.T) {
	t.Parallel()
	result := services.MakeIPerfResultFull(
		"udp",
		"upload",
		900.0,
		1125.0,
...
func TestMakeVLANConfigFullHelper(t *testing.T) {
	t.Parallel()
	cfg := services.MakeVLANConfigFull(
		100,
		"Management",
		"eth0",
		"192.168.100.1",
...
func TestMakeLinkStatusFullHelper(t *testing.T) {
	t.Parallel()
	status := services.MakeLinkStatusFull(
		"eth0",
		services.LinkStateUp,
		"1000Mbps",
		"full",
...
func TestMakeTelemetrySnapshotFullHelper(t *testing.T) {
	...
	links := []services.LinkStatus{
		services.MakeLinkStatus("eth0", services.LinkStateUp, "1000Mbps", "full", 1500, "00:11:22:33:44:55"),
	}
	gateway := &services.GatewayHealth{IP: "192.168.1.1", Reachable: true, Status: services.HealthStatusHealthy}
	bandwidth := &services.BandwidthSample{Interface: "eth0", TxMbps: 100, RxMbps: 50}
	systemHealth := &services.SystemHealth{CPUPercent: 25.0, MemoryPercent: 60.0}

	snapshot := services.MakeTelemetrySnapshotFull(links, gateway, nil, nil, bandwidth, systemHealth)
...
func TestMakeSystemHealthFullHelper(t *testing.T) {
	t.Parallel()
	health := services.MakeSystemHealthFull(25.0, 60.0, 45.0, 55.0, 24*time.Hour*30, []float64{1.5, 2.0, 1.8})
