...
func TestDefaultInterfaceConstant(t *testing.T) {
	if services.DefaultInterfaceConst != "eth0" {
		t.Errorf("expected DefaultInterface 'eth0', got %q", services.DefaultInterfaceConst)
	}
}

func TestInterfaceStateWaitMsConstant(t *testing.T) {
	if services.InterfaceStateWaitMsConst != 100 {
		t.Errorf("expected InterfaceStateWaitMs 100, got %d", services.InterfaceStateWaitMsConst)
	}
}

func TestSNMPTimeticksPerSecondConstant(t *testing.T) {
	if services.SNMPTimeticksPerSecondConst != 100 {
		t.Errorf("expected SNMPTimeticksPerSecond 100, got %d", services.SNMPTimeticksPerSecondConst)
	}
}

func TestDefaultIPerfPortConstant(t *testing.T) {
	if services.DefaultIPerfPortConst != 5201 {
		t.Errorf("expected DefaultIPerfPort 5201, got %d", services.DefaultIPerfPortConst)
	}
}

func TestDefaultIPerfDurationSecConstant(t *testing.T) {
	if services.DefaultIPerfDurationSecConst != 10 {
		t.Errorf("expected DefaultIPerfDurationSec 10, got %d", services.DefaultIPerfDurationSecConst)
	}
}
...
func TestLinkStateValues(t *testing.T) {
	tests := []struct {
		...
		{"up", services.LinkStateUp, "up"},
		{"down", services.LinkStateDown, "down"},
		{"dormant", services.LinkStateDormant, "dormant"},
		{"unknown", services.LinkStateUnknown, "unknown"},
	}
...
func TestCableStatusValues(t *testing.T) {
	tests := []struct {
		...
		{"ok", services.CableStatusOK, "ok"},
		{"open", services.CableStatusOpen, "open"},
		{"short", services.CableStatusShort, "short"},
		{"impedance", services.CableStatusImpedance, "impedance_mismatch"},
		{"unknown", services.CableStatusUnknown, "unknown"},
	}
...
func TestHealthStatusValues(t *testing.T) {
	tests := []struct {
		...
		{"healthy", services.HealthStatusHealthy, "healthy"},
		{"degraded", services.HealthStatusDegraded, "degraded"},
		{"unhealthy", services.HealthStatusUnhealthy, "unhealthy"},
		{"unknown", services.HealthStatusUnknown, "unknown"},
	}
...
func TestLinkStatusMakeHelper(t *testing.T) {
	status := services.MakeLinkStatus("eth0", services.LinkStateUp, "1000", "full", 1500, "00:11:22:33:44:55")
...
func TestCableTestResultFields(t *testing.T) {
	...
	result := services.CableTestResult{
		...
		PairResults: []services.PairResult{
			{Pair: 1, Status: services.CableStatusOK, Length: 25.0},
			{Pair: 2, Status: services.CableStatusOK, Length: 25.2},
			{Pair: 3, Status: services.CableStatusOK, Length: 25.3},
			{Pair: 4, Status: services.CableStatusOK, Length: 25.8},
		},
		TestedAt: now,
	}
...
func TestPairResultFields(t *testing.T) {
	...
	if pair.Status != services.CableStatusOK {
...
func TestDHCPTestResultAllFields(t *testing.T) {
	now := time.Now()
	result := services.DHCPTestResult{
		Success:      true,
...
func TestDNSTestResultMakeHelper(t *testing.T) {
	result := services.MakeDNSTestResult("google.com", "8.8.8.8", true, 10.0)
...
func TestSpeedtestResultAllFields(t *testing.T) {
	now := time.Now()
	result := services.SpeedtestResult{
		DownloadMbps: 100.0,
		UploadMbps:   50.0,
		PingMs:       10.0,
		JitterMs:     2.0,
...
func TestSpeedtestResultMakeHelper(t *testing.T) {
	result := services.MakeSpeedtestResult(100.0, 50.0, 10.0, "Test Server")
...
func TestIPerfResultMakeHelper(t *testing.T) {
	result := services.MakeIPerfResult("tcp", "download", 500.0, 625.0, 10.0, "192.168.1.100")
...
func TestSNMPDeviceMakeHelper(t *testing.T) {
	device := services.MakeSNMPDevice("192.168.1.1", "switch-01", "Cisco IOS")
...
func TestBandwidthSampleMakeHelper(t *testing.T) {
	sample := services.MakeBandwidthSample("eth0", 1000.0, 500.0, 75.0)
...
func TestSystemHealthMakeHelper(t *testing.T) {
	health := services.MakeSystemHealth(25.0, 50.0, 75.0)
...
func TestTelemetrySnapshotAllFields(t *testing.T) {
	now := time.Now()
	gwHealth := &services.GatewayHealth{IP: "192.168.1.1", Reachable: true}
	dnsResult := &services.DNSTestResult{Success: true}
	dhcpResult := &services.DHCPTestResult{Success: true}
	bandwidth := &services.BandwidthSample{Interface: "eth0", TxMbps: 100.0}
	sysHealth := &services.SystemHealth{CPUPercent: 25.0}

	snapshot := services.TelemetrySnapshot{
		Timestamp: now,
		Links: []services.LinkStatus{
			{Interface: "eth0", State: services.LinkStateUp},
		},
		Gateway:      gwHealth,
		DNS:          dnsResult,
		DHCP:         dhcpResult,
...
func TestConvertGatewayStatus(t *testing.T) {
	tests := []struct {
		...
		{"invalid", "invalid", services.HealthStatusUnknown},
	}
...
func TestSapErrors(t *testing.T) {
	tests := []struct {
		...
		{"ErrNotImplemented", services.ErrNotImplemented, "not implemented: pending migration"},
		{"ErrNotInitialized", services.ErrNotInitialized, "service not initialized"},
		{"ErrNotSupported", services.ErrNotSupported, "feature not supported on this platform"},
		{"ErrTestFailed", services.ErrTestFailed, "test failed"},
	}
...
func TestConcurrentLinkStatusAccess(_ *testing.T) {
	...
	for range 10 {
		go func() {
			for range 50 {
				_ = services.MakeLinkStatus("eth0", services.LinkStateUp, "1000", "full", 1500, "00:11:22:33:44:55")
			}
			done <- true
		}()
	}
...
func TestConcurrentSpeedtestResultAccess(_ *testing.T) {
	...
	for range 10 {
		go func() {
			for range 50 {
				_ = services.MakeSpeedtestResult(100.0, 50.0, 10.0, "Test Server")
			}
			done <- true
		}()
	}
...
func TestLinkStatusTableDriven(t *testing.T) {
	tests := []struct {
		...
		{"up interface", "eth0", services.LinkStateUp, "1000", 1500, true},
		{"down interface", "eth1", services.LinkStateDown, "0", 1500, false},
		{"dormant interface", "eth2", services.LinkStateDormant, "1000", 1500, false},
		{"unknown interface", "eth3", services.LinkStateUnknown, "", 1500, false},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			status := services.MakeLinkStatus(tt.iface, tt.state, tt.speed, "full", tt.mtu, "00:00:00:00:00:00")
...
func TestSpeedtestResultTableDriven(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := services.MakeSpeedtestResult(tt.download, tt.upload, tt.ping, "Test Server")
...
func TestIPerfResultTableDriven(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := services.MakeIPerfResult(tt.protocol, tt.direction, tt.bandwidth, 125.0, 10.0, "localhost")
...
func TestGatewayHealthTableDriven(t *testing.T) {
	tests := []struct {
		...
		{"healthy gateway", "192.168.1.1", true, 2.0, 0.0, services.HealthStatusHealthy},
		{"degraded gateway", "192.168.1.1", true, 50.0, 5.0, services.HealthStatusDegraded},
		{"unhealthy gateway", "192.168.1.1", false, 0.0, 100.0, services.HealthStatusUnhealthy},
		{"unknown gateway", "10.0.0.1", false, 0.0, 0.0, services.HealthStatusUnknown},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			health := services.MakeGatewayHealth(tt.ip, tt.reachable, tt.rtt, tt.packetLoss, tt.status)
