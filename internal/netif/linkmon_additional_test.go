...
func TestLinkStateStringDefaultCase(t *testing.T) {
	...
	invalidState := netif.LinkState(99)
	...
	negativeState := netif.LinkState(-1)
...
func TestLinkMonitorRecordEvent(t *testing.T) {
	mon := netif.NewLinkMonitor("eth0")
	helper := netif.NewLinkMonitorTestHelper(mon)

	// Add events and verify history
	event1 := netif.LinkEvent{
		Interface: "eth0",
		State:     netif.LinkStateUp,
		Timestamp: time.Now().Add(-1 * time.Hour),
	}
	event2 := netif.LinkEvent{
		Interface: "eth0",
		State:     netif.LinkStateDown,
		Timestamp: time.Now(),
	}
	...
	if history[0].State != netif.LinkStateUp {
		t.Errorf("First event state = %v, want Up", history[0].State)
	}
	if history[1].State != netif.LinkStateDown {
...
func TestLinkMonitorProcessStateChangeNoChange(t *testing.T) {
	...
	// Set initial state
	helper.SetLastState(netif.LinkStateUp)

	// Process same state - should not change
	shouldNotify, changed := helper.ProcessStateChange(netif.LinkStateUp)
	if changed {
...
func TestLinkMonitorProcessStateChangeWithChange(t *testing.T) {
	...
	// Set initial state
	helper.SetLastState(netif.LinkStateDown)
	...
	if helper.GetLastState() != netif.LinkStateUp {
...
func TestLinkMonitorSafeInvokeCallback(t *testing.T) {
	...
	callback := func(e netif.LinkEvent) {
		called.Store(true)
		if e.Interface != "eth0" {
...
func TestLinkMonitorSafeInvokeCallbackWithPanic(t *testing.T) {
	...
	panicCallback := func(_ netif.LinkEvent) {
...
func TestLinkMonitorNotifyCallbacks(t *testing.T) {
	...
	callbacks := make([]netif.LinkStateCallback, 3)
	for i := range 3 {
		wg.Add(1)
		callbacks[i] = func(_ netif.LinkEvent) {
			callCount.Add(1)
			wg.Done()
		}
	}
...
func TestLinkMonitorGetFlapCount24hWithEvents(t *testing.T) {
	...
	helper.AddHistoryEvent(netif.LinkEvent{
		Interface: "eth0",
		State:     netif.LinkStateUp,
...
func TestLinkMonitorWaitForLinkUpTimeout(t *testing.T) {
	...
	mon := netif.NewLinkMonitor("nonexistent999")
...
func TestLinkMonitorCheckLinkState(t *testing.T) {
	...
	validStates := []netif.LinkState{
		netif.LinkStateUp,
		netif.LinkStateDown,
		netif.LinkStateUnknown,
	}
...
func TestLinkMonitorCheckLinkStateNonExistent(t *testing.T) {
	...
	state := helper.CheckLinkState()
	if state != netif.LinkStateUnknown {
...
func TestLinkMonitorMultipleCallbacks(t *testing.T) {
	...
	for i := range 5 {
		idx := i
		mon.OnStateChange(func(_ netif.LinkEvent) {
			mu.Lock()
...
func TestLinkEventTimestamp(t *testing.T) {
	...
	if event.State != netif.LinkStateUp {
...
func TestLinkMonitorSetInterfaceWhileRunning(t *testing.T) {
	...
	if helper.GetLastState() != netif.LinkStateUnknown {
