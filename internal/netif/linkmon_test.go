...
func TestLinkStateString(t *testing.T) {
	tests := []struct {
		...
		{"up state", netif.LinkStateUp, "up"},
		{"down state", netif.LinkStateDown, "down"},
		{"unknown state", netif.LinkStateUnknown, "unknown"},
	}
...
func TestLinkMonitorOnStateChange(t *testing.T) {
	...
	var receivedEvent netif.LinkEvent

	// Register callback.
	mon.OnStateChange(func(event netif.LinkEvent) {
		callbackCalled = true
		receivedEvent = event
	})
...
func TestLinkStateConstants(t *testing.T) {
	...
	if netif.LinkStateUnknown == netif.LinkStateDown ||
		netif.LinkStateUnknown == netif.LinkStateUp {
		t.Error("LinkStateUnknown overlaps with other states")
	}

	if netif.LinkStateDown == netif.LinkStateUp {
		t.Error("LinkStateDown == LinkStateUp")
	}

	// Verify they have different string representations.
	states := map[netif.LinkState]string{
		netif.LinkStateUnknown: "unknown",
		netif.LinkStateDown:    "down",
		netif.LinkStateUp:      "up",
	}
